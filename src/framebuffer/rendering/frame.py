...
class Frame:
    def __init__(self, size: tuple[int, int]) -> None:
        ...
        self.colour_buffer: ColourBuffer = [[Vector.zeros(4).astype(int)] * self.size[0] for _ in range(self.size[1])]
    ...
    def clear(self) -> None:
        for yi in range(self.size[1]):
            for xi in range(self.size[0]):
                self.colour_buffer[yi][xi] = Vector.zeros(4).astype(int)
                self.depth_buffer[yi][xi] = math.inf

    def set_size(self, size: tuple[int, int]) -> None:
        self.size = size
        self.colour_buffer = [[Vector.zeros(4).astype(int)] * self.size[0] for _ in range(self.size[1])]
